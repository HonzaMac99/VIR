#https://github.com/mseitzer/pytorch-fid/tree/master/src/pytorch_fid
...
class ImagePathDataset(torch.utils.data.Dataset):
    def __init__(self, files, transforms=None):
        ...
        self.transforms = Lambda(lambda y: y.float())
    ...
    def __getitem__(self, i):
        ...
        img = torch.tensor(np.array(PILImage)).permute(2, 0, 1)
        if self.transforms is not None:
            #img = TF.Resize(64)(img)
...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu', num_workers=1):
    ...
    for batch in dataloader:
        ...
        #plt.axis("off")
        #grid_img = make_grid(batch[:rows * cols].cpu(), nrow=rows, padding=1, pad_value=1)
        #grid_img = grid_img.permute(1, 2, 0)
        #IMG_PATH = f'G:\\VIR2021\\fid_AA\\000.png'
